...
def add_scan(scann:UploadFile, size:str, unit_s:str, weight:str,unit_w:str, owner_id:int,db: Session = Depends(get_db)):
    ...
    new=crud.create_scan(db=db, scan=scan,owner_id=owner_id)
    processing.image_processing(new.id,db)
    return new
...
@app.get("/insole/{scan_id}", response_model=schemas.Insole)
def read_insole_by_scan_id(scan_id: int, db: Session = Depends(get_db)):
    db_user = crud.get_insoles_by_scan_id(db=db, scan_id=scan_id)
    if db_user is None:
        raise HTTPException(status_code=404, detail="Scan not found")
    return db_user

@app.get("/insole/{insole_id}", response_model=schemas.Insole)
def read_insole_by_insole_id(insole_id: int, db: Session = Depends(get_db)):
    db_user = crud.get_insoles_by_insole_id(db=db, insole_id=insole_id)
    if db_user is None:
        raise HTTPException(status_code=404, detail="Scan not found")
    return db_user
