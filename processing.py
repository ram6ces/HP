...
def image_processing(scan_id, db):
    ...
    print(scan_id)
    try:
        scan=crud.get_scan_by_scan_id(db=db,scan_id=scan_id+1)
        print(scan.scann)
...
def process():
    ...
    try:
        max_scan_id = SessionLocal().query(func.max(models.Scan.id)).scalar()
        print(max_scan_id)
        max_insole = SessionLocal().query(func.max(models.Insole.id)).scalar()
        print(max_insole)
        ...
        for scan_id in range(max_scan_id):
            try:
                if crud.get_insoles_by_scan_id(db=SessionLocal(), scan_id=scan_id) == None:
                    ...
                    crud.create_insole(db=SessionLocal,insole=insole, scanner_id=scan_id)
