...
def get_name_scann(db:Session,scan_id):
    db_scan=get_scan_by_scan_id(db=db,scan_id=scan_id)
    
    return db_scan.scann
...
def create_insole(db: Session, insole: schemas.Insole,scanner_id:int):
    db_insole=models.Insole(data=insole.data, scanner_id=insole.scanner_id)
    sess=db()
    sess.add(db_insole)
    sess.commit()
    sess.refresh(db_insole)
    print(db_insole)
    return db_insole
...
def get_insoles_by_insole_id(db:Session, insole_id:int):
    return db.query(models.Insole).filter(models.Scan.id == insole_id).first()
    #return db.query(models.Insole).join(models.Scan).filter(models.Insole.id == insole_id).first()
